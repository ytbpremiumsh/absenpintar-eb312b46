...
function Nav() {
  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-all ${scrolled ? "backdrop-blur-md bg-[#0b1020]/80 border-b border-white/5" : "bg-transparent"}`}>
      <div className="max-w-7xl mx-auto px-6 lg:px-10 h-16 flex items-center justify-between">
        <a href="/" className="flex items-center gap-2 group">
          <div className="h-8 w-8 rounded-lg bg-[#5B6CF9] flex items-center justify-center shadow-lg shadow-[#5B6CF9]/30">
      ...
      {open && (
        <div className="lg:hidden bg-[#0b1020] border-t border-white/5 px-6 py-4 space-y-3">
          {NAV.map((n) => (
            <a key={n.href} href={n.href} onClick={() => setOpen(false)} className="block text-white/80 py-2 text-sm font-medium">
              {n.label}
            </a>
          ))}
          <div className="pt-3 border-t border-white/5 flex gap-2">
            <button onClick={() => nav("/login")} className="flex-1 text-sm font-semibold text-white/90 border border-white/15 rounded-lg py-2.5">
              Masuk
            </button>
      ...
      <div className="grid lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p className="mt-5 text-base lg:text-lg text-[#0b1020]/70 leading-relaxed">
            ATSkolla tidak mengenakan biaya lisensi kepada sekolah. Pendapatan ATSkolla berasal dari layanan tambahan seperti pembayaran online, kartu pelajar, dan perangkat pendukung sekolah — sehingga sekolah dapat menggunakan platform tanpa biaya berlangganan.
          </p>
